...
promise which creates a new image (use document.createElement('img')) and sets the .src attribute to 
the provided image path. When the image is done loading, append it to the DOM element with the 
...
earlier;
4. After the 2 seconds have passed, hide the current image (set display to 'none'), and load a 
second image (HINT: Use the image element returned by the createImage promise to hide the current 
...
const loadAll = async function(imgArr) {
    try{
        ...
        imgsEL.map(img => img.classList.add('parallel'))
        console.log(imgsEL);
    }catch(err) {
        console.error(err.message)
    }
